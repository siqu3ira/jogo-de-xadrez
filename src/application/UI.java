...
public class UI {
    ...
    public static PosicaoDeXadrez lerPosicaoDeXadrez(Scanner sc) {
        try {
            ...
            String posicao = sc.nextLine().toLowerCase();
            char coluna = posicao.charAt(0);
            int linha = Integer.parseInt(posicao.substring(1));

            return new PosicaoDeXadrez(coluna, linha);
        } catch (RuntimeException e) {
            throw new InputMismatchException("Erro ao ler a posição. Posições válidas são de a1 a h8.");
        }
    }

    public static void printPartida(Partida partida, List<PecaDeXadrez> pecasCapturadas) {
        printTabuleiro(partida.getPecas());
        System.out.println();
        printPecasCapturadas(pecasCapturadas);
        System.out.println();
